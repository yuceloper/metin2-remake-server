...
public sealed class LegacyCharacterBootstrapPublisher(
    ...
    ILegacyCharacterBootstrapRuntimeContextProvider runtimeContextProvider) : ILegacyCharacterBootstrapPublisher
{
    ...
    public async ValueTask PublishAsync(GameSession session, CancellationToken cancellationToken = default)
    {
        ...
        if (runtime.Points.Length != PointCount)
        {
            throw new InvalidOperationException($"Legacy point projection must contain exactly {PointCount} values.");
        }

        if (runtime.Parts.Length != PartCount)
        {
            throw new InvalidOperationException($"Legacy character update requires exactly {PartCount} parts.");
        }

        if (runtime.Affects.Length != AffectCount)
        {
            throw new InvalidOperationException($"Legacy character update requires exactly {AffectCount} affect words.");
        }
        ...
        PacketFrameWriteStatus detailsStatus = PacketFrameWriter.TryWrite(
            in details,
            destination[offset..],
            out int detailsWritten);
        EnsureWritten(nameof(CharacterDetails), detailsStatus, detailsWritten, CharacterDetailsFrameSize);
        offset += detailsWritten;

        PacketFrameWriteStatus pointsStatus = PacketFrameWriter.TryWrite(
            in pointPacket,
            destination[offset..],
            out int pointsWritten);
        EnsureWritten(nameof(CharacterPoints), pointsStatus, pointsWritten, CharacterPointsFrameSize);
        offset += pointsWritten;

        PacketFrameWriteStatus updateStatus = PacketFrameWriter.TryWrite(
            in update,
            destination[offset..],
            out int updateWritten);
        EnsureWritten(nameof(CharacterUpdate), updateStatus, updateWritten, CharacterUpdateFrameSize);
        offset += updateWritten;
    ...
    {
        points[1] = snapshot.Level;
        points[3] = snapshot.Experience;
        points[11] = snapshot.Gold;
        points[12] = snapshot.Strength;
        points[13] = snapshot.Vitality;
        points[14] = snapshot.Dexterity;
        points[15] = snapshot.Intelligence;
        points[26] = snapshot.AvailableStatusPoints;
    }
